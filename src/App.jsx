...
function TaskDrawerContent({ task: taskItem, state, showChecklistOnCard, onToggleChecklistOnCard, onClose, onSave, onDelete, onComment, onAttachment, onOpenQuote, onAddSubtask, onRequestDelete, deleteState, showDeletePrompt, onCancelDelete, onConfirmDelete }) {
  ...
  const mentionSuggestions = mentionMatch ? (state.employees || []).filter((employee) => normalizeText(employee.name).includes(mentionQuery)).slice(0, 6) : [];
  ...
  const insertMention = (employee) => { const match = comment.match(/(^|\s)@([^\s@]*)$/); if (!match) return; setComment(`${comment.slice(0, match.index + match[1].length)}@${employee.name} `); };
  const handleSave = () => {
