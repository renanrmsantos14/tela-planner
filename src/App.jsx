...
const CENTRAL_NAV_ITEMS = [
  ["dashboard", "Minhas pendências", LayoutDashboard], ["team", "Equipe", Users], ["board", "Tarefas", ClipboardList], ["list", "Lista", ListFilter], ["calendar", "Agenda", CalendarDays], ["quality", "Qualidade", ShieldAlert], ["settings", "Configurações", Settings],
];

const MOBILE_NAV_ITEMS = [
  ["dashboard", "Início", LayoutDashboard], ["board", "Quadro", ClipboardList], ["list", "Lista", ListFilter], ["calendar", "Agenda", CalendarDays], ["quality", "Qualidade", ShieldAlert], ["team", "Equipe", Users], ["settings", "Mais", Settings],
];
...
function AppShell({ active, onNavigate, children, onCreate, tasks, live, currentEmployee, personalStats, openTaskCount }) {
  ...
  return <div className={`app-shell ${expanded ? "" : "sidebar-collapsed"}`}>
    <aside className="sidebar">
      ...
      <nav className="main-nav">{CENTRAL_NAV_ITEMS.map(([id, label, Icon]) => <button key={id} className={active === id ? "nav-item active" : "nav-item"} onClick={() => onNavigate(id)}><Icon size={18} /><span>{label}</span>{id === "board" && <span className="nav-count">{openTaskCount}</span>}{id === "dashboard" && personalStats?.alertCount > 0 && <span className="nav-count nav-count-alert">{personalStats.alertCount}</span>}</button>)}</nav>
...
function QualityView({ state, onCreate, onCreateTask, filters, setFilters }) {
  ...
  const filteredQuality = useMemo(() => quality.filter((item) => {
    ...
    const matchesTaskFilters = linkedTask ? filterTasks([linkedTask], taskFilters).length > 0 : !filters.assignee?.length && !filters.priority?.length && !filters.source?.length && !filters.team && !filters.blocked;
...
function TaskDrawerContent({ task: taskItem, state, currentEmployee, showChecklistOnCard, onToggleChecklistOnCard, onClose, onSave, onDelete, onComment, onAttachment, onOpenQuote, onAddSubtask, onRequestDelete, deleteState, showDeletePrompt, onCancelDelete, onConfirmDelete }) {
  const [form, setForm] = useState(taskItem ? { ...taskItem } : null); const [comment, setComment] = useState(""); const [mentionActiveIndex, setMentionActiveIndex] = useState(0); const [newSubtaskTitle, setNewSubtaskTitle] = useState(""); const [isAddingSubtask, setIsAddingSubtask] = useState(false); const [subtaskToDelete, setSubtaskToDelete] = useState(null); const [saveState, setSaveState] = useState("idle"); const [validationError, setValidationError] = useState(""); const [showDiscardPrompt, setShowDiscardPrompt] = useState(false); const assigneeOptions = useMemo(() => buildAssigneeOptions(state.employees), [state.employees]);
  const saveCloseTimerRef = useRef(null);
  useEffect(() => {
    setForm(taskItem ? { ...taskItem, assigneeName: normalizeAssigneeNames(taskItem.assigneeNames || taskItem.assigneeName) } : null); setComment(""); setMentionActiveIndex(0); setNewSubtaskTitle(""); setIsAddingSubtask(false); setSubtaskToDelete(null); setSaveState("idle"); setValidationError(""); setShowDiscardPrompt(false);
  ...
  const mentionQuery = normalizeText(mentionMatch?.[1] || "");
   ...
   const isDirty = ["title", "status", "priority", "teamName", "dueDate", "description", "blockedReason"].some((key) => JSON.stringify(form[key] || "") !== JSON.stringify(taskItem[key] || "")) || JSON.stringify(form.assigneeName || []) !== JSON.stringify(normalizeAssigneeNames(taskItem.assigneeNames || taskItem.assigneeName)) || comment.length > 0;
  ...
  const submitSubtask = () => { const title = newSubtaskTitle.trim(); if (!title) return; onAddSubtask(taskItem.id, title); setNewSubtaskTitle(""); setIsAddingSubtask(false); };
   ...
   const handleSave = () => {
     ...
     setValidationError("");
     setSaveState("saving");
     onSave(taskItem.id, { title: form.title, status: form.status, priority: form.priority, assigneeNames: form.assigneeName, teamName: form.teamName, dueDate: form.dueDate, description: form.description, blockedReason });
...
function NewTaskDrawer({ quotes, employees = [], onClose, onSave, onSearchQuotes = (query) => plannerQuoteSearch?.(query) }) {
  const [form, setForm] = useState(() => { const linkedQuoteId = launchQuoteId(); return { title: "", quoteId: quotes.some((item) => item.id === linkedQuoteId) ? linkedQuoteId : "", priority: "medium", assigneeName: [], teamName: "Comercial", dueDate: "", description: "" }; }); const [subtasks, setSubtasks] = useState([]); const initialFormRef = useRef(form); const [showDiscardPrompt, setShowDiscardPrompt] = useState(false); const [saveState, setSaveState] = useState("idle"); const saveCloseTimerRef = useRef(null); const assigneeOptions = useMemo(() => buildEmployeeAssigneeOptions(employees), [employees]); const quoteOptions = useMemo(() => quotes.map((item) => ({ value: item.id, label: `${item.code} · ${item.title}`, search: item.client, _remoteSearch: onSearchQuotes })), [quotes, onSearchQuotes]); const quote = quotes.find((item) => item.id === form.quoteId);
...
export default function App() {
  ...
  const connectGraph = useCallback(() => { if (!store.connectGraph) return Promise.resolve(false); return runMutation(store.connectGraph(), "Microsoft 365 conectado."); }, [store, runMutation]);
  ...
  const renderPage = () => {
    if (active === "dashboard" || active === "team") return <CentralView state={viewState} mode={active === "dashboard" ? "mine" : "team"} currentEmployee={currentEmployee} onOpenTask={openTask} onOpenSource={(item) => store.openSource?.(item)} onCompleteTask={completeTask} onCreate={() => setCreating(true)} />;
