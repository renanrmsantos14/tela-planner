...
function DeleteTaskDialog({ taskTitle, onCancel, onConfirm }) {
  return <div className="drawer-confirm-layer" onMouseDown={(event) => event.stopPropagation()}><div className="drawer-confirm" role="dialog" aria-modal="true" aria-labelledby="delete-task-title"><h2 id="delete-task-title">Excluir tarefa?</h2><p>“{taskTitle}” será removida do Planner. Esta ação não pode ser desfeita.</p><div className="drawer-confirm-actions"><button className="button button-quiet" type="button" onClick={onCancel}>Cancelar</button><button className="button button-danger" type="button" onClick={onConfirm}>Excluir tarefa</button></div></div></div>;
}
...
function Board({ tasks, onOpen, onMove, onCreate }) {
...
function TaskDrawerContent({ task: taskItem, state, onClose, onSave, onComment, onAttachment, onOpenQuote, onAddSubtask }) {
  const [form, setForm] = useState(taskItem ? { ...taskItem } : null); const [comment, setComment] = useState(""); const [newSubtaskTitle, setNewSubtaskTitle] = useState(""); const [isAddingSubtask, setIsAddingSubtask] = useState(false); const [saveState, setSaveState] = useState("idle"); const [showDiscardPrompt, setShowDiscardPrompt] = useState(false);
  const saveCloseTimerRef = useRef(null);
  useEffect(() => {
    setForm(taskItem ? { ...taskItem } : null); setComment(""); setNewSubtaskTitle(""); setIsAddingSubtask(false); setSaveState("idle"); setShowDiscardPrompt(false);
...
export default function App() {
  const [active, setActive] = useState("dashboard"); const [state, setState] = useState(null); const [store] = useState(() => createDataStore()); const [selectedId, setSelectedId] = useState(""); const [creating, setCreating] = useState(false); const [filters, setFilters] = useState({ query: "", assignee: [], priority: [], source: [] }); const [notice, setNotice] = useState(""); const [error, setError] = useState(""); const [failedTaskDraft, setFailedTaskDraft] = useState(null);
  ...
  const saveTask = useCallback((id, patch) => {
    ...
    return runOptimisticMutation((current) => applyOptimisticTaskPatch(current, id, patch), () => store.updateTask(state, id, patch), store.live ? "Alterações salvas na tela. Sincronizando..." : "Alterações salvas no mock local.", store.live ? "Tarefa sincronizada." : "Tarefa atualizada localmente.").then((success) => {
      if (success) {
  ...
  const deleteTask = useCallback((id) => {
    runOptimisticMutation((current) => ({ ...current, tasks: current.tasks.filter((taskItem) => taskItem.id !== id) }), () => store.deleteTask(state, id), store.live ? "Tarefa excluída. Sincronizando..." : "Tarefa excluída do mock local.", store.live ? "Exclusão sincronizada." : "Tarefa excluída.").then((success) => { if (success) setSelectedId(""); });
