...
function CentralTaskDrawer({ onClose, onSave, employees = [], initialContext = null }) {
  const [form, setForm] = useState({ title: initialContext?.title || "", priority: initialContext?.priority || "medium", assigneeName: "NÃ£o atribuÃ­do", teamName: "OperaÃ§Ã£o", dueDate: initialContext?.dueDate || "", description: initialContext?.description || "" });
...
export default function App() {
  ...
  const showNotice = useCallback((message, duration = 2600) => { setNotice(message); if (noticeTimerRef.current) window.clearTimeout(noticeTimerRef.current); noticeTimerRef.current = window.setTimeout(() => setNotice(""), duration); }, []);
  ...
  const runOptimisticMutation = useCallback((update, operation, pendingMessage, successMessage) => {
    ...
    Promise.resolve().then(operation).then((next) => {
