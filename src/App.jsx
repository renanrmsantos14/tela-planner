...
function fileToDataUrl(file) {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onerror = () => reject(reader.error || new Error("Não foi possível preparar o arquivo local."));
...
function createDraftAttachment(file, prefix = "draft") {
  ...
  return { id: `${prefix}-${globalThis.crypto?.randomUUID?.() || `${Date.now()}-${Math.random()}`}`, name: file.name, mimeType: file.type, size: file.size, previewUrl, file, syncStatus: "pending" };
}
...
function AttachmentSection({ taskId, attachments = [], loadAttachmentContent, onAttachment, onDeleteAttachment, helperText }) {
  const inputId = useId();
...
function TaskDrawer({ task, onDelete, ...props }) {
  ...
  return <TaskDrawerContent task={task} onDelete={onDelete} onRequestDelete={() => setShowDeletePrompt(true)} deleteState={deleteState} showDeletePrompt={showDeletePrompt} onCancelDelete={() => setShowDeletePrompt(false)} onConfirmDelete={confirmDelete} loadAttachmentContent={props.loadAttachmentContent || globalThis.__plannerAttachmentLoader} {...props} />;
}

function NewTaskDrawer({ employees = [], onClose, onSave }) {
  const [form, setForm] = useState({ title: "", priority: "medium", assigneeName: [], teamName: "Comercial", dueDate: "", description: "" });
  ...
  const isDirty = Object.keys(initialFormRef.current).some((key) => JSON.stringify(form[key]) !== JSON.stringify(initialFormRef.current[key])) || draftAttachments.length > 0;
  const requestClose = () => { if (saveState !== "idle") return; if (isDirty) setShowDiscardPrompt(true); else onClose(); };
  const set = (key, value) => setForm((current) => ({ ...current, [key]: value }));
  const handleDraftAttachment = (id, filesOrFile) => setDraftAttachments((current) => [...current, ...toAttachmentFiles(filesOrFile).map((file) => createDraftAttachment(file, "new"))]);
...
export default function App() {
  ...
  const addAttachment = useCallback((id, filesOrFile) => {
