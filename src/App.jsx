...
function AttachmentPreview({ attachment, loadAttachmentContent }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [attachment, cacheKey, loadAttachmentContent]);
  if (!isImageAttachment(attachment)) return null;
  if (state.dataUrl) return <div className="attachment-preview"><img src={state.dataUrl} alt={attachment.name || "Imagem anexada"} loading="lazy" /></div>;
  if (state.loading) return <small className="attachment-preview-state">Carregando imagem...</small>;
  if (state.error) return <small className="attachment-preview-state attachment-preview-error">Prévia indisponível</small>;
  return null;
}
