...
function CollectionFocusRow({ row, employees, today, onOpenTask, onCollect }) {
  ...
  return <article className="management-focus-row"><div className="management-focus-row-copy"><div><strong>{row.title}</strong><span className={`priority priority-${priority.tone}`}>{priority.label}</span></div><ResponsibleDisplay row={row} employees={employees} /><span className="management-focus-due"><AlertTriangle size={13} />{daysLabel(row.overdueDays)} em atraso · {formatDate(row.dueDate)}</span></div><div className="management-focus-actions"><button className="button button-secondary button-small" type="button" onClick={() => onOpenTask(row.id)}>Abrir</button><button className="button button-primary button-small" type="button" disabled={row.collectionDate === today} onClick={() => onCollect(row.id)}>{row.collectionDate === today ? "Cobrada hoje" : "Cobrar"}</button></div></article>;
}
