...
import { AlertTriangle, ArrowLeft, ArrowUpRight, CalendarDays, CheckCircle2, ChevronRight, Clock3, EyeOff, RotateCcw, Users, UserRound } from "lucide-react";
import { formatDate, getDueBucket, isTaskVisibleToEmployee, priorityById } from "./domain.js";
...
export default function ManagementView({ state, currentEmployee, onOpenTask, onCollect, onRegisterWaitingReturn }) {
  const [view, setView] = useState("overview");
  const visibleTasks = useMemo(() => (state.tasks || []).filter((task) => isTaskVisibleToEmployee(task, currentEmployee, state.teams)), [state.tasks, state.teams, currentEmployee]);
  const collections = useMemo(() => collectionRows(visibleTasks, state.employees, state.teams, state.collectionEvents, new Date()), [visibleTasks, state.employees, state.teams, state.collectionEvents]);
  const workload = useMemo(() => workloadGroups(visibleTasks, state.teams, new Date()), [visibleTasks, state.teams]);
  const workloadTotal = useMemo(() => workloadTotals(visibleTasks, new Date()), [visibleTasks]);
  const waiting = useMemo(() => waitingRows(visibleTasks, state.teams, new Date()), [visibleTasks, state.teams]);
...
function CollectionFocusRow({ row, employees, today, onOpenTask, onCollect }) {
  const priority = priorityById(row.priority);
  return <article className="management-focus-row"><div className="management-focus-row-copy"><div className="task-title-inline"><strong>{row.title}</strong>{row.restrictedVisibility && <span className="task-visibility-mark" role="img" title="Visível somente para responsáveis e criador" aria-label="Tarefa oculta: visível somente para responsáveis e criador"><EyeOff size={13} strokeWidth={2.2} aria-hidden="true" /></span>}<span className={`priority priority-${priority.tone}`}>{priority.label}</span></div><ResponsibleDisplay row={row} employees={employees} /><span className="management-focus-due"><AlertTriangle size={13} />{daysLabel(row.overdueDays)} em atraso · {formatDate(row.dueDate)}</span></div><div className="management-focus-actions"><button className="button button-secondary button-small" type="button" onClick={() => onOpenTask(row.id)}>Abrir</button><button className="button button-primary button-small" type="button" disabled={row.collectionDate === today} onClick={() => onCollect(row.id)}>{row.collectionDate === today ? "Cobrada hoje" : "Cobrar"}</button></div></article>;
}
...
function CollectionTable({ rows, employees = [], today, onOpenTask, onCollect }) {
  return <section className="panel management-panel"><div className="panel-heading"><div><span className="eyebrow">PRAZO ESTOURADO</span><h2>Central de cobrança</h2></div><span className="panel-count">{rows.length}</span></div>{rows.length ? <div className="management-list">{rows.map((row) => { const priority = priorityById(row.priority); return <article className="management-row" key={row.id}><div className="management-row-main"><div className="management-row-title task-title-inline"><strong>{row.title}</strong>{row.restrictedVisibility && <span className="task-visibility-mark" title="Visível somente para responsáveis e criador" aria-label="Tarefa oculta: visível somente para responsáveis e criador"><EyeOff size={13} strokeWidth={2.2} aria-hidden="true" /></span>}<span className={`priority priority-${priority.tone}`}>{priority.label}</span></div><ResponsibleDisplay row={row} employees={employees} /><small>Criada por <strong>{row.creatorName}</strong></small></div><div className="management-row-due"><strong className="danger-text">{daysLabel(row.overdueDays)} em atraso</strong><span><CalendarDays size={14} />{formatDate(row.dueDate)}</span>{row.lastCollectionAt && <small>Última cobrança: {new Intl.DateTimeFormat("pt-BR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" }).format(new Date(row.lastCollectionAt))}</small>}</div><div className="management-row-actions"><button className="button button-secondary button-small" type="button" onClick={() => onOpenTask(row.id)}><ArrowUpRight size={14} />Abrir</button><button className="button button-primary button-small" type="button" disabled={row.collectionDate === today} onClick={() => onCollect(row.id)}>{row.collectionDate === today ? "Cobrada hoje" : "Cobrar agora"}</button></div></article>; })}</div> : <EmptyState icon={CheckCircle2} title="Nenhuma tarefa atrasada" detail="A Central de cobrança está em dia." />}</section>;
}
