...
export default function CentralView({ state, mode = "mine", currentEmployee, onOpenTask, onOpenSource, onCreate, onTransition }) {
  ...
  const transition = (item, status) => {
    ...
    onTransition?.(item.sourceRecordId, status);
    window.setTimeout(() => setTransitioningId(""), 260);
  };
  ...
  return <div className="page-content">
    ...
    <section className="panel central-panel">
      ...
      <div className="central-filters" role="search" aria-label="Filtros de pendências">
        ...
        <label className="central-filter"><span>Origem</span><select value={filters.source} onChange={(event) => updateFilter("source", event.target.value)}><option value="all">Todas as origens</option><option value="task">Tarefas</option><option value="quote_followup">Acompanhamentos</option><option value="quality_error">Ocorrências</option><option value="quality_action">Ações</option></select></label>
        <label className="central-filter"><span>Responsável</span><select value={filters.assignee} onChange={(event) => updateFilter("assignee", event.target.value)}><option value="all">Toda a equipe</option>{assignees.map((employee) => <option value={employee.id || employee.name} key={employee.id || employee.name}>{employee.name}</option>)}</select></label>
      ...
      <div className="central-list">
        {items.map((item) => <CentralRow key={item.id} item={item} onOpen={open} onTransition={transition} transitioning={transitioningId === item.id} />)}
...
function CentralRow({ item, onOpen, onTransition, transitioning }) {
  const canTransition = item.source === "task" || item.source === "quote_followup";
  ...
  return <article className="central-row">
    ...
    {canTransition && item.quickTransitions?.length > 0 && <div className="central-quick-actions" aria-label={`Ações para ${item.title}`}><button type="button" disabled={transitioning || item.statusGroup === "doing"} onClick={() => onTransition(item, "doing")}>Iniciar</button><button type="button" disabled={transitioning || item.statusGroup === "waiting"} onClick={() => onTransition(item, "waiting")}>Aguardar</button></div>}
  </article>;
}
