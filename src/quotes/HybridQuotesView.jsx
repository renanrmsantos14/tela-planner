...
import { filterQuotes, getQuoteMetrics, isQuoteOpen, QUOTE_OPEN_STATUSES, QUOTE_PRIORITIES, QUOTE_TERMINAL_STATUSES, validateQuoteCommercial } from "../quoteDomain";
...
function MoveDialog({ move, task, saving, onCancel, onConfirm, onAttachment, onDeleteAttachment, loadAttachmentContent, AttachmentSectionComponent }) {
  ...
  return <div className="quote-v3-confirm-layer"><form className={`quote-v3-dialog${commercial ? " quote-commercial-dialog" : ""}`} role="dialog" aria-modal="true" onSubmit={(event) => { event.preventDefault(); if ((!loss || reason.trim()) && (!commercial || validCommercial) && (move.status !== "Respondida ao cliente" || sentConfirmed)) onConfirm(reason.trim(), commercial ? { value, commercialTerms, ...(move.status === "Respondida ao cliente" ? { responseSent: true } : {}) } : {}); }}><header className="quote-dialog-header"><div><span className="quote-dialog-eyebrow">Atualização da cotação</span><h3>{commercial ? "Informar dados comerciais" : terminal ? `Mover para ${move.status}?` : "Alterar status?"}</h3></div><button className="icon-button" type="button" onClick={onCancel} aria-label="Fechar"><X size={18} /></button></header><p>{commercial ? "Registre o valor e as condições antes de avançar." : terminal ? "A cotação e a tarefa vinculada serão encerradas." : `${move.quote.code} passará para ${move.status}.`}</p>{commercial && <><div className="quote-dialog-fields"><label htmlFor="quote-move-value">Valor total (BRL)<FormMoneyInput id="quote-move-value" autoFocus required value={value} onChange={(event) => setValue(event.target.value)} placeholder="R$ 0,00" /></label><label htmlFor="quote-move-terms">Condições comerciais<FormTextArea id="quote-move-terms" rows="4" value={commercialTerms} onChange={(event) => setCommercialTerms(event.target.value)} placeholder="Ex.: pagamento, validade e observações da proposta" /></label></div>{move.status === "Respondida ao cliente" && <label className="quote-dialog-checkbox"><input type="checkbox" checked={sentConfirmed} onChange={(event) => setSentConfirmed(event.target.checked)} /><span>Confirmo que a proposta já foi enviada ao cliente.</span></label>}{task && AttachmentSectionComponent && <div className="quote-dialog-attachments"><div className="quote-dialog-section-title"><span>Anexos da cotação</span><small>Opcional</small></div><AttachmentSectionComponent taskId={task.id} attachments={task.attachments || []} loadAttachmentContent={loadAttachmentContent} onAttachment={onAttachment} onDeleteAttachment={onDeleteAttachment} itemLabel="à cotação" helperText="" showPreview={false} allowOpen={false} compact /></div>}</>}{loss && <label htmlFor="quote-move-reason">Motivo da perda<FormTextArea id="quote-move-reason" autoFocus required rows="4" value={reason} onChange={(event) => setReason(event.target.value)} /></label>}<div className="quote-dialog-actions"><button className="button button-secondary" type="button" onClick={onCancel}>Voltar</button><button className="button button-primary" type="submit" disabled={saving || (loss && !reason.trim()) || (commercial && !validCommercial) || (move.status === "Respondida ao cliente" && !sentConfirmed)}>{saving ? "Atualizando…" : "Confirmar"}</button></div></form></div>;
}
...
export default function HybridQuotesView({ state, currentEmployee, WaitingContextFieldsComponent, ReturnsSectionComponent, onOpenTask, onCreateQuote, onUpdateQuote, onDeleteQuote, onRequestWaitingQuote, onRegisterWaitingReturn, onMarkQuoteSent, onSetQuoteOutcome, selectedQuoteId, onSelectQuote, onEnsureTaskDetails, onAttachment, onDeleteAttachment, loadAttachmentContent, AttachmentSectionComponent }) {
  ...
  return <div className="page-content quotes-page quote-v3-page" data-view="quotes"><div className="page-header"><div><span className="eyebrow">ACOMPANHAMENTO COMERCIAL</span><h1>Cotações</h1><p>Cadastre, priorize e acompanhe a resposta sem sair da gestão.</p></div><button className="button button-primary" type="button" onClick={() => setCreating(true)}><Plus size={15} />Nova cotação</button></div>
    ...
    <section className="panel quote-v3-panel"><div className={`quote-v3-toolbar${filtersExpanded ? " is-expanded" : ""}`}>
      ...
      <SearchableMultiSelect value={filters.status} onChange={(value) => setFilter("status", value)} options={QUOTE_OPEN_STATUSES.map((status) => ({ value: status, label: status }))} placeholder="Todos os status" />
