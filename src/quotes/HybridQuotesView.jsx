...
import { CalendarDays, FileText, LayoutGrid, List, Plus, Search, UserRound } from "lucide-react";
...
function MoveDialog({ move, saving, onCancel, onConfirm }) {
  ...
  return <div className="quote-v3-confirm-layer"><form className="quote-v3-dialog" role="dialog" aria-modal="true" onSubmit={(event) => { event.preventDefault(); if (!loss || reason.trim()) onConfirm(reason.trim()); }}><h3>{terminal ? `Mover para ${move.status}?` : "Alterar status?"}</h3><p>{terminal ? "A cotação e a tarefa vinculada serão encerradas." : `${move.quote.code} passará para ${move.status}.`}</p>{loss && <label htmlFor="quote-move-reason">Motivo da perda<textarea id="quote-move-reason" autoFocus required value={reason} onChange={(event) => setReason(event.target.value)} /></label>}<div><button className="button button-secondary" type="button" onClick={onCancel}>Voltar</button><button className="button button-primary" type="submit" disabled={saving || (loss && !reason.trim())}>{saving ? "Atualizando…" : "Confirmar"}</button></div></form></div>;
}
