...
export default function HybridQuotesView({ state, onOpenTask, onCreateQuote, onUpdateQuote, onMarkQuoteSent, onSetQuoteOutcome, selectedQuoteId, onSelectQuote, onEnsureTaskDetails, onAttachment, onDeleteAttachment, loadAttachmentContent, AttachmentSectionComponent }) {
  ...
  return <div className="page-content quotes-page quote-v3-page" data-view="quotes"><div className="page-header"><div><span className="eyebrow">ACOMPANHAMENTO COMERCIAL</span><h1>Cotações</h1><p>Cadastre, priorize e acompanhe a resposta sem sair da gestão.</p></div><button className="button button-primary" type="button" onClick={() => setCreating(true)}><Plus size={15} />Nova cotação</button></div>
    ...
    {selectedQuote && <QuoteManagementDrawer quote={selectedQuote} task={selectedTask} employees={state.employees || []} onClose={() => onSelectQuote?.("")} onOpenTask={(id) => { onSelectQuote?.(""); onOpenTask?.(id); }} onEnsureTaskDetails={onEnsureTaskDetails} onUpdate={onUpdateQuote} onMarkSent={onMarkQuoteSent} onOutcome={onSetQuoteOutcome} onAttachment={onAttachment} onDeleteAttachment={onDeleteAttachment} loadAttachmentContent={loadAttachmentContent} AttachmentSectionComponent={AttachmentSectionComponent} />}
