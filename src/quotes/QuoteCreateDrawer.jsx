...
const INITIAL_DRAFT = { status: "Nova", priority: "medium", assigneeIds: [], assigneeNames: [] };
...
export default function QuoteCreateDrawer({ employees = [], onClose, onCreate, AttachmentSectionComponent }) {
  ...
  const validateCurrent = () => {
    ...
    if (!result.valid) setTimeout(() => document.getElementById(`quote-${Object.keys(result.errors)[0]?.replace("clientContact", "contact").replace("clientPhone", "phone").replace("clientEmail", "email").replace("serviceDate", "service-date")}`)?.focus(), 0);
    return result.valid;
  };
  const next = () => {
    if (!validateCurrent()) return;
    if (stepIndex === QUOTE_CREATE_STEPS.length - 1) submit();
  ...
  const submit = async (confirmedWithoutDeadline = false) => {
    ...
    if (!result.valid) {
      ...
      setStepIndex(STEP_BY_FIELD[firstField] ?? 0);
      setTimeout(() => document.getElementById(`quote-${firstField?.replace("clientContact", "contact").replace("clientPhone", "phone").replace("clientEmail", "email").replace("serviceDate", "service-date")}`)?.focus(), 0);
      return;
    }
  ...
  return <div className="quote-v3-layer" onKeyDown={keepTabInDrawer} onMouseDown={(event) => { if (event.target === event.currentTarget) requestClose(); }}>
    <aside className="quote-v3-drawer quote-v3-create" role="dialog" aria-modal="true" aria-labelledby="quote-create-title">
      ...
      <div className="quote-v3-drawer-body" onPaste={handlePaste}>
        <><div className="quote-v3-step-progress" role="progressbar" aria-valuemin="1" aria-valuemax={QUOTE_CREATE_STEPS.length} aria-valuenow={stepIndex + 1}><span style={{ width: `${((stepIndex + 1) / QUOTE_CREATE_STEPS.length) * 100}%` }} /></div>
          <div className="quote-v3-step-heading"><span>Etapa {stepIndex + 1} de {QUOTE_CREATE_STEPS.length}</span><h3>{step.label}</h3></div></>
      ...
      <footer className="quote-v3-drawer-footer"><div>{stepIndex > 0 && <button className="button button-secondary" type="button" onClick={() => setStepIndex((current) => current - 1)} disabled={saving}><ArrowLeft size={15} />Voltar</button>}<button className="button button-primary" type="button" onClick={next} disabled={saving}>{stepIndex === QUOTE_CREATE_STEPS.length - 1 ? (saving ? <><span className="quote-v3-spinner" />Criando cotação…</> : "Confirmar e criar") : <><span>Avançar</span><ArrowRight size={15} /></>}</button></div></footer>
    </aside>
...
const STEP_BY_FIELD = { title: 0, client: 0, clientContact: 0, clientPhone: 0, clientEmail: 0, origin: 1, destination: 1, notes: 1, serviceDate: 1, deadline: 2 };
